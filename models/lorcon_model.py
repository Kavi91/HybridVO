...
class LoRCoNFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.simple_conv1 = nn.Conv2d(10, 32, kernel_size=3, stride=(1, 2), padding=(1, 0))
        self.conv_bn1 = nn.BatchNorm2d(32)
        self.simple_conv2 = nn.Conv2d(32, 64, kernel_size=3, stride=(1, 2), padding=(1, 0))
        self.conv_bn2 = nn.BatchNorm2d(64)
        self.simple_conv3 = nn.Conv2d(64, 128, kernel_size=3, stride=(1, 2), padding=(1, 0))
        self.conv_bn3 = nn.BatchNorm2d(128)
        self.simple_conv4 = nn.Conv2d(128, 256, kernel_size=3, stride=(2, 1), padding=(1, 0))  # Adjust stride
        self.conv_bn4 = nn.BatchNorm2d(256)
        self.simple_conv5 = nn.Conv2d(256, 512, kernel_size=3, stride=(2, 1), padding=(1, 0))  # Adjust stride
    ...
    def encode_image(self, x):
        ...
        x = self.conv_bn1(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv1: {x.shape}")
        x = F.pad(x, (1, 1, 0, 0), mode='circular')
        x = self.simple_conv2(x)
        x = self.conv_bn2(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv2: {x.shape}")
        x = F.pad(x, (1, 1, 0, 0), mode='circular')
        x = self.simple_conv3(x)
        x = self.conv_bn3(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv3: {x.shape}")
        x = F.pad(x, (1, 1, 0, 0), mode='circular')
        x = self.simple_conv4(x)
        x = self.conv_bn4(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv4: {x.shape}")
        x = F.pad(x, (1, 1, 0, 0), mode='circular')
        x = self.simple_conv5(x)
        x = self.conv_bn5(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv5: {x.shape}")
        x = F.pad(x, (1, 1, 0, 0), mode='circular')
        x = self.simple_conv6(x)
        x = self.conv_bn6(x)
        x = F.leaky_relu(x, 0.1); #print(f"conv6: {x.shape}")
        # Adjust width to 76 without padding
        target_width = 76
